 ...
 * @param {number} percent - overall completion percentage
 * @param {number} completed - completed task count
 * @param {number} inProgress - in progress task count
 * @param {number} review - needs review task count
 * @param {number} pending - pending task count
 * @param {number} total - total task count
 * @param {string[]} agents - agent names working on this
 * @param {boolean} paused - whether work is paused
 * @param {boolean} animate - animate progress bars on mount
 * @param {function} onViewObjective - optional callback for "View Objective" button
...
export function ObjectiveCard({
  ...
  return (
    <div className={`rounded-2xl bg-[var(--color-surface)] shadow-lg shadow-black/5 ${className}`} style={{ outline: '1px solid var(--color-border)', outlineOffset: '0px' }}>
      <div className="px-6 py-5">
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-[var(--color-border)]">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-[var(--color-muted)]">{agents.join(' & ')} are working on this</span>
          </div>
          <div className="flex items-center gap-3">
